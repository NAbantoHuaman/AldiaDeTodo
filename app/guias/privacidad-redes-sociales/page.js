...
import { EyeOff, ArrowRight, CheckCircle, AlertTriangle, Info } from 'lucide-react';
...
export const metadata = {
  ...
  keywords: ['seguridad', 'privacidad', 'redes sociales'],
  openGraph: {
...
export default function Guia() {
  ...
  {
    id: "economia",
    title: "La economía de tus datos"
  },
  {
    id: "auditoria",
    title: "Auditoría de tu huella digital"
  },
  {
    id: "configuracion",
    title: "Configuraciones críticas por red"
  },
  {
    id: "compartir",
    title: "Qué NO compartir nunca"
  },
  {
    id: "limpieza",
    title: "Limpiando el pasado digital"
  }
];
    ...
    <article className="min-h-screen bg-white font-inter text-slate-900">
      ...
      <header className="relative bg-slate-950 text-white overflow-hidden">
        ...
        <div className="container mx-auto max-w-4xl px-4 py-24 md:py-32 relative z-10">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-black font-outfit leading-[0.9] tracking-tight mb-8">
            Privacidad en Redes Sociales en 2026<br/><span className="text-red-400"></span>
          </h1>
          <p className="text-xl text-slate-300 max-w-2xl leading-relaxed font-medium">
            Audita y protege tus perfiles sociales. Descubre qué datos están recolectando sobre ti y cómo limitar tu exposición digital. En esta guía completa y detallada, exploraremos las mejores prácticas y estrategias probadas en la actualidad.
          </p>
        </div>
      </header>
      ...
      <div className="container mx-auto max-w-4xl px-4 py-16 md:py-24">
        <nav className="bg-slate-50 rounded-3xl p-8 mb-16 border border-slate-100">
          <h2 className="text-lg font-black text-slate-900 mb-6 flex items-center gap-3"><EyeOff className="w-5 h-5 text-red-500" /> Contenido de la Guía</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {sections.map((s, i) => (
              <a key={s.id} href={`#${s.id}`} className="flex items-center gap-3 text-sm text-slate-600 hover:text-red-600 transition-colors group">
                <span className="w-7 h-7 bg-white rounded-lg border border-slate-200 flex items-center justify-center text-[10px] font-black text-slate-400 group-hover:bg-red-500 group-hover:text-white transition-all">{i + 1}</span>
                {s.title}
              </a>
            ))}
          </div>
        ...
        <div className="prose prose-lg prose-slate max-w-none">
          {sections.map((s, idx) => (
            <section key={s.id} id={s.id} className="mb-12">
              <h2 className="text-3xl font-black font-outfit mt-0 flex items-center gap-3"><CheckCircle className="w-7 h-7 text-red-500" /> {s.title}</h2>
        ...
        <div className="bg-red-600 rounded-[40px] p-10 md:p-16 mt-24 text-white relative shadow-2xl overflow-hidden">
          ...
          <div className="relative z-10 text-center md:text-left">
            ...
            <p className="text-red-100 text-xl leading-relaxed mb-12 font-medium max-w-2xl">Esperamos que esta guía te brinde el impulso necesario. Sigue explorando nuestro catálogo para continuar tu aprendizaje.</p>
