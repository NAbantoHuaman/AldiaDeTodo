...
export async function generateMatchAnalysis(matchData) {
  if (!process.env.GOOGLE_API_KEY) return "Análisis no disponible (Falta API Key).";

  try {
    const prompt = `
      Actúa como un analista táctico de fútbol para Al Día de Todo.
      Analiza este partido en vivo y da una perspectiva experta.
      
      Partido: ${matchData.home} ${matchData.score} ${matchData.away}
      Tiempo: ${matchData.elapsed}' (${matchData.status})
      Eventos Recientes: ${JSON.stringify(matchData.events)}
      Liga: ${matchData.league}

      Genera un análisis breve (máximo 120 palabras) que incluya:
      1. Tónica del partido (quién domina).
      2. Qué significa el marcador actual para ambos.
      3. Un pronóstico breve para lo que resta.
      
      Usa un tono profesional, emocionante y usa <b>negritas</b> para puntos clave.
    ...
    return "Error al generar el análisis táctico.";
  }
}
